...
{#if waitingOnVerif}
    ...
    <div>
        We sent you an email with verification link{#if exUser}to the address <b
                >{exUser}</b
            >{/if}. Just click the link and this page will be automatically
        update to complete your sign in. No special code needed.
    </div>
...
{:else}
    ...
    <form on:submit|preventDefault={() => (signUp ? signUpFB() : signInFB())}>
        ...
        {:else}
            <div class="verif" class:complete={hasMinimumLength}>
                {#if hasMinimumLength}&check;{:else}&times;{/if} Password must be
    ...
    <div>--or--</div>
    <button on:click={loginWithGoogleRedirect}>Log in with Google</button>
    <div>--or--</div>

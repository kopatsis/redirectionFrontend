<script>
    ...
    async function fetchData() {
        ...
        if (searchParam)
            queryParams.append("q", encodeURIComponent(searchParam));
...
<section>
    ...
    {:else}
        {#if paying}
            ...
            {:else}
                <div>
                    <button on:click={fetchcsv}
                        >Export CSV of all shortened links</button
                    >
                </div>
            {/if}
            {#if csverror}
                <div>
                    Error fetching CSV, may be a server issue or lack of
                    internet connection...
                </div>
            {/if}
        {/if}
